...
- [x] Development tranche (TDD + ponytail):
  ...
  - [x] T4 verification: automated live gate `go test -tags live ./plugins/mini-docker/` passes against the real daemon — daemon reachable and all three trees validate; observed 11 containers / 6 running on the audit laptop (logged by the test, not asserted). Manifest 0.3.0, panel width 480 (live-named decision), fallback synced, binaries rebuilt
- [x] Independent tranche review (2026-09-22) fix-first items landed: the live-gate claim above corrected; every client write serialized under one send mutex (publish vs panel-open host.call); `view.resync` handled with a revision reset, pinned by `TestViewResyncResetsRevisionAndRepublishes`
